...
const PrismBackground: React.FC<PrismProps> = ({
  ...
  useEffect(() => {
    ...
    const WHITE_GLOW = Math.max(0.0, whiteGlow || 1);
    const RSX = 1;
    const RSY = 1;
    const RSZ = 1;
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        float distToCenter = length(gl_FragCoord.xy - 0.5 * iResolution.xy);
        float centerGlow = 1.0 - smoothstep(0.0, iResolution.y * 0.4, distToCenter);
        vec3 whiteGlow = vec3(1.0, 1.0, 1.0) * centerGlow * uWhiteGlow * 0.5;
        col += whiteGlow;
        
        // 添加全局环境光，提升整体亮度
        col += vec3(0.05, 0.05, 0.08) * uWhiteGlow;
